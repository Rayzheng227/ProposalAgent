...
class ProposalAgent:
    ...
    def execute_step_node(self, state: ProposalState) -> ProposalState:
        ...
        try:
            if action_name == "search_arxiv_papers":
                ...
                QueueUtil.push_mes(StreamMes(state["proposal_id"], 3, "\n正在使用ArXiv工具搜索相关论文，请耐心等待~"))
                result = search_arxiv_papers_tool.invoke(parameters)
                # 将结果保存到状态中
                if isinstance(result, list) and len(result) > 0:
                    state["arxiv_papers"].extend(result)
                    QueueUtil.push_mes(StreamMes(state["proposal_id"], 3,
                                                 f"\n✅ ArXiv工具调用结束，耗时 {time.time() - start_timestamp:.2f} 秒，共找到 {len(result)} 篇论文"))

            elif action_name == "search_web_content":
                start_timestamp = time.time()
                QueueUtil.push_mes(StreamMes(state["proposal_id"], 3, "\n正在使用网页搜索工具查找相关资源，请耐心等待~"))
                result = search_web_content_tool.invoke(parameters)
                # 将结果保存到状态中
                if isinstance(result, list) and len(result) > 0:
                    state["web_search_results"].extend(result)
                    QueueUtil.push_mes(StreamMes(state["proposal_id"], 3,
                                                 f"\n✅ 网页搜索工具调用结束，耗时 {time.time() - start_timestamp:.2f} 秒, 共找到 {len(result)} 篇资源"))

            elif action_name == "search_crossref_papers":
                start_timestamp = time.time()
                QueueUtil.push_mes(StreamMes(state["proposal_id"], 3, "\n正在使用CrossRef工具搜索相关论文，请耐心等待~"))
                result = search_crossref_papers_tool.invoke(parameters)
                # 将结果保存到状态中
                if isinstance(result, list) and len(result) > 0:
                    state["web_search_results"].extend(result)
                    QueueUtil.push_mes(StreamMes(state["proposal_id"], 3,
                                                 f"\n✅ CrossRef工具调用结束，耗时 {time.time() - start_timestamp:.2f} 秒，共找到 {len(result)} 篇论文"))
                ...
                QueueUtil.push_mes(StreamMes(state["proposal_id"], 3,
                                             f"\n✅ 论文管理操作结束，耗时 {time.time() - start_timestamp:.2f} 秒"))
    ...
    def generate_proposal(self, research_field: str, proposal_id: str) -> Dict[str, Any]:
        """生成研究计划书"""
